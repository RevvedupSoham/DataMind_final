const HARD_BLOCK_PATTERNS = [
  ...
  /\b(information_schema\.roles?|pg_catalog\.pg_authid)\b/i,
...
export function validateSqlSafety(sql: string): SqlValidationResult {
  ...
  if (
    !/^(select|with|insert|update|delete|create|alter|drop|truncate)\b/i.test(
      statement
    )
  ) {
    return {
  ...
  if (
    /^(update|delete)\b/i.test(statement) &&
